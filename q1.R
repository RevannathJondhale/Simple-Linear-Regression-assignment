...
#correlation between the given X and Y
cor(Y,X)    #correlation value =  0.8259973

#create the model for predicting the Y value
...
x = log(Sorting.Time); y = log(Delivery.Time)
...
ggplot(data = DT, aes(x = X, y = Y)) + 
  ...
  geom_line(color='blue',data = DT, aes(x= X, y= pred))
...
# check the correlation between the given X and Y
...
#model for predicting the Y value
...
######10101010##############Model
# Polynomial model with 4 degree with logarithmic Y (quadratic model)
...
###12-12-12#############Model
#Polynomial model with 5 degree with logarithmic Y (quadratic model)
...
###14-14-14####
#Polynomial model with 6 degree with logarithmic Y (quadratic model)
...
# Data shown in the attached Excel datasheet we can conclude that the model no. 12 is the best model with which we have got the best results.
# Result is based on the values of Correlation, R-Squared value and RMSE of the given data to get the most optimum result.
# We are going to use the model no. 12 for the required predictions.