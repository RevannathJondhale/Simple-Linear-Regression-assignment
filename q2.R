...
ggplot(data = SH, aes(x = X, y = Y)) + 
  ...
  geom_line(color='black',data = SH, aes(x=X, y=pred))
...
# Modelno- 2 -  x = log(YearsExperience); y = Salary
X<-YearsExperience;y<-Salary
plot(log(x),y)
cor(log(X),y) # correlation value-0.9240611
mod_log <- lm(Y ~ log(X))   # lm(Y ~ X)
summary(mod_log)#R-squared:  0.8539
pred<-predict(mod_log)
pred
mod_log$residuals
sqrt(mean(mod_log$residuals^2))##10302.89 RMSE
...
ggplot(data = SH, aes(x = log(X), y = Y))+geom_point(color='blue') +
